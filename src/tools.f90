...
contains
  ...
  subroutine test_scalar_min_max(phi)
    ...
    integer :: code,ierror,i,j,k,is,icht
    real(mytype) :: phimax,phimin,phimax1,phimin1
    real(mytype),dimension(xsize(1),xsize(2),xsize(3),numscalar) :: phi
    character(len=15)  :: string1,string2

401 format('Phi', I2,' min max=')
402 format('PhiS',I2,' min max=')
    ...
    do is=1, numscalar
       ...
       if (nrank.eq.0) then
          ...
          write (string1,401) is
          print*,string1, real(phimin1,4), real(phimax1,4)
       ...
       if (itbc(is).eq.3) then !CHT
           icht=icht+1
           phimax = maxval(phis1(:,:,:,icht))
           phimin = minval(phis1(:,:,:,icht))
           call MPI_REDUCE(phimax,phimax1,1,real_type,MPI_MAX,0,MPI_COMM_WORLD,code)
           call MPI_REDUCE(phimin,phimin1,1,real_type,MPI_MIN,0,MPI_COMM_WORLD,code)
           if (nrank.eq.0) then
              !print *,'PhiS'//char(48+is)//' min max=', real(phimin1,4), real(phimax1,4)
              write (string2,402) is
              print*,string2, real(phimin1,4), real(phimax1,4)
              if (abs(phimax1).ge.10.) then !if phi control turned off
                 print *,'Solid Scalar diverged! SIMULATION IS STOPPED!'
                 call MPI_ABORT(MPI_COMM_WORLD,code,ierror); stop
              endif
           endif
       endif
  ...
  subroutine restart(ux1,uy1,uz1,dux1,duy1,duz1,ep1,pp3,phi1,dphi1,px1,py1,pz1,iresflg)
    ...
    USE var, ONLY: phis1, dphis1

    implicit none

    integer :: i,j,k,iresflg,nzmsize,fh,ierror,is,it,code,icht
    ...
    if (iresflg==1) then !write
       ...
       if (iscalar==1) then
          ...
          enddo
          !If Conjugate Heat Transfer, write solid temperature fields
          if (numcht.ne.0) then 
             do icht=1,numcht
                call decomp_2d_write_var(fh,disp,1,phis1(:,:,:,icht))
                ! previous time-steps
                if ((itimescheme.eq.2).or.(itimescheme.eq.3).or.(itimescheme.eq.7)) then ! AB2 or AB3
                  call decomp_2d_write_var(fh,disp,1,dphis1(:,:,:,2,icht))
                end if
                !
                if ((itimescheme.eq.3).or.(itimescheme.eq.7)) then ! AB3
                  call decomp_2d_write_var(fh,disp,1,dphis1(:,:,:,3,icht))
                end if
             enddo
          endif
       endif
       ...
       !If Conjugate Heat Transfer, read solid temperature fields
       if (numcht.ne.0) then 
          do icht=1,numcht
             call decomp_2d_read_var(fh,disp,1,phis1(:,:,:,icht))
             ! previous time-steps
             if ((itimescheme.eq.2).or.(itimescheme.eq.3).or.(itimescheme.eq.7)) then ! AB2 or AB3
               call decomp_2d_read_var(fh,disp,1,dphis1(:,:,:,2,icht))
             end if
             !
             if ((itimescheme.eq.3).or.(itimescheme.eq.7)) then ! AB3
               call decomp_2d_read_var(fh,disp,1,dphis1(:,:,:,3,icht))
             end if
          enddo
       endif
