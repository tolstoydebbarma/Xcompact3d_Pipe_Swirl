...
module pipe
  ...
  PUBLIC :: init_pipe, boundary_conditions_pipe, postprocess_pipe, postprocess_pipe_extra, pipe_flrt, pipe_blkt, &
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  !********************************************************************
    ...
    call MPI_ALLREDUCE(ncount,ncountt,1,real_type,MPI_SUM,MPI_COMM_WORLD,code)
    qmm=qmm/ncountt
    if (nrank==0 .and. mod(itime,ilist)==0) print *,'    Mean velocity  after',qmm
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  !********************************************************************
	...
	character(len=30) :: filename
	character(len=11), parameter :: foldname_stat='statistics/'
    ...
    if (mod(itime,icheckpoint)==0) then
        ...
        write(filename,"(A,I8.8)") trim(foldname_stat) // 'pmean.dat',itime
        ta1(1,:,:)=pmean(:,:)
        call decomp_2d_write_plane(1,ta1,1,1,filename)
        
		!write(filename,"('umean.dat',I7.7)") itime
        write(filename,"(A,I8.8)") trim(foldname_stat) // 'umean.dat',itime
		ta1(1,:,:)=umean(:,:)
        call decomp_2d_write_plane(1,ta1,1,1,filename)
        
		!write(filename,"('vmean.dat',I7.7)") itime
		write(filename,"(A,I8.8)") trim(foldname_stat) // 'vmean.dat',itime
        ta1(1,:,:)=vmean(:,:)
        call decomp_2d_write_plane(1,ta1,1,1,filename)
        
		!write(filename,"('wmean.dat',I7.7)") itime
        write(filename,"(A,I8.8)") trim(foldname_stat) // 'wmean.dat',itime
		ta1(1,:,:)=wmean(:,:)
        call decomp_2d_write_plane(1,ta1,1,1,filename)
		
        !write(filename,"('uumean.dat',I7.7)") itime
		write(filename,"(A,I8.8)") trim(foldname_stat) // 'uumean.dat',itime
        ta1(1,:,:)=uumean(:,:)
        call decomp_2d_write_plane(1,ta1,1,1,filename)
        
		!write(filename,"('vvmean.dat',I7.7)") itime
		write(filename,"(A,I8.8)") trim(foldname_stat) // 'vvmean.dat',itime
        ta1(1,:,:)=vvmean(:,:)
        call decomp_2d_write_plane(1,ta1,1,1,filename)
        
		!write(filename,"('wwmean.dat',I7.7)") itime
		write(filename,"(A,I8.8)") trim(foldname_stat) // 'wwmean.dat',itime
        ta1(1,:,:)=wwmean(:,:)
        call decomp_2d_write_plane(1,ta1,1,1,filename)
        
		!write(filename,"('uvmean.dat',I7.7)") itime
		write(filename,"(A,I8.8)") trim(foldname_stat) // 'uvmean.dat',itime
        ta1(1,:,:)=uvmean(:,:)
        call decomp_2d_write_plane(1,ta1,1,1,filename)
        
		!write(filename,"('uwmean.dat',I7.7)") itime
		write(filename,"(A,I8.8)") trim(foldname_stat) // 'uwmean.dat',itime
        ta1(1,:,:)=uwmean(:,:)
        call decomp_2d_write_plane(1,ta1,1,1,filename)
        
		!write(filename,"('vwmean.dat',I7.7)") itime
		write(filename,"(A,I8.8)") trim(foldname_stat) // 'vwmean.dat',itime
        ta1(1,:,:)=vwmean(:,:)
        call decomp_2d_write_plane(1,ta1,1,1,filename)
		
        !write(filename,"('kmean.dat',I7.7)") itime
		write(filename,"(A,I8.8)") trim(foldname_stat) // 'kmean.dat',itime
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !!
  !!  SUBROUTINE: postprocess_pipe_extra
  !!      AUTHOR: Tolstoy Debbarma
  !! DESCRIPTION: Extra Statistics collection for pipe
  !!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !********************************************************************
  !
  SUBROUTINE postprocess_pipe_extra(ux1,uy1,uz1,ep1)
    ...
    use variables, only : sx
    use variables, only : sy
    use variables, only : sz
	
    USE var, only : ta1,tb1,tc1,td1,te1,tf1,tg1,th1,ti1,di1
    USE var, only : ta2,tb2,tc2,td2,te2,tf2,di2,ta3,tb3,tc3,td3,te3,tf3,di3
	USE var, only : dudxsum,utmapsum
	...
	if (mod(itime,icheckpoint)==0) then
		...
		!FIRST DERIVATIVE
		dudxsum(:,:,:) = ta1(:,:,:)		!dudxsum = dudxsum + tsum
887		format(a,'dudx.sum',I8.8)
		write(filename, 887) trim(foldname_stat), itime
		call decomp_2d_write_one(1,dudxsum,filename,1)
		
		!FRICTION VELOCITY OVER ITERATION
		di1 = zero
		di1(:,:,:) = sqrt(sqrt( td1(:,:,:)**2 + tf1(:,:,:)**2 )*xnu) 		!du/dy**2 + dw/dy**2
		
		utmapsum(:,:,:) = ta1(:,:,:)			!utmapsum=utmapsum+utsum
		write(filename,"(A,I8.8)") trim(foldname_stat) // 'utmap.sum',itime
		call decomp_2d_write_plane(1,utmapsum,2,1,filename)
	endif
	return
  end SUBROUTINE postprocess_pipe_extra
  !********************************************************************
  !
  !********************************************************************
  !
  SUBROUTINE momentum_forcing_pipe(duy1, duz1, uy1, uz1, ep1)
  !
  !********************************************************************
	USE ibm_param, ONLY: ra
	...
	real(mytype)                                        :: ym,zm,yc,zc,r, theta_t, F_theta				! tolstoy---swirling turbulent pipe
	
    REAL(mytype), INTENT(IN), DIMENSION(xsize(1), xsize(2), xsize(3)) :: uy1, uz1, ep1
    REAL(mytype), DIMENSION(xsize(1), xsize(2), xsize(3), ntime) :: duy1, duz1
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  USE MPI
  USE ibm
  USE time_integrators, only: intt							! comment for make file
  USE var, ONLY : ep1
  ...
  integer                                                  :: i,j,k,iibm_save
    ...
    !=============== TIME ADVANCEMENT ===============
    CALL intt(phis1(:,:,:), dphis1(:,:,:,:))							 ! comment for make file
