...
module variables
  ...
  real(mytype),allocatable,dimension(:) :: sc,uset,cp,ri,group,itbc,g1,g2
  integer :: numcht !if conjugate heat transfer 
  ...
  !velocity
  real(mytype),allocatable,dimension(:) :: vfx,vsx,vwx
  real(mytype),allocatable,dimension(:) :: vcx,vbx,vfxp,vsxp,vwxp
  real(mytype),allocatable,dimension(:) :: vfy,vsy,vwy
  real(mytype),allocatable,dimension(:) :: vcy,vby,vfyp,vsyp,vwyp
  real(mytype),allocatable,dimension(:) :: vfz,vsz,vwz
  real(mytype),allocatable,dimension(:) :: vcz,vbz,vfzp,vszp,vwzp
  !scalar 
  real(mytype),allocatable,dimension(:,:) :: vscfx,vscsx,vscwx
  real(mytype),allocatable,dimension(:,:) :: vscfy,vscsy,vscwy
  real(mytype),allocatable,dimension(:,:) :: vscfz,vscsz,vscwz
  real(mytype),allocatable,dimension(:)   :: vsccx,vscbx,vscfxp,vscsxp,vscwxp
  real(mytype),allocatable,dimension(:)   :: vsccy,vscby,vscfyp,vscsyp,vscwyp
  real(mytype),allocatable,dimension(:)   :: vsccz,vscbz,vscfzp,vscszp,vscwzp
  !scalar CHT (solid coefficients) 
  real(mytype),allocatable,dimension(:,:) :: vscfx_s,vscsx_s,vscwx_s
  real(mytype),allocatable,dimension(:,:) :: vscfy_s,vscsy_s,vscwy_s
  real(mytype),allocatable,dimension(:,:) :: vscfz_s,vscsz_s,vscwz_s
...
!  real(mytype), allocatable,dimension(:) :: rrmt0,qqmt0,vvmt0,ssmt0,l1mt,l2mt,l3mt,u1mt,u2mt,u3mt !!TIME IMPLICIT SCALAR (with HPL), ncl=0
  ...
  ABSTRACT INTERFACE
     ...
     SUBROUTINE FILTER_XX(t,u,r,fval,fva,fvb,fvc,fvd,fve,&
                          s,vf,vs,vw,nx,ny,nz,npaire)
       use decomp_2d, only : mytype
       integer :: nx,ny,nz,npaire
       real(mytype), dimension(nx,ny,nz) :: t,u,r
       real(mytype), dimension(ny,nz):: s
       real(mytype), dimension(nx):: vf,vs,vw
       real(mytype)               :: fval,fva,fvb,fvc,fvd,fve
     END SUBROUTINE FILTER_XX
     SUBROUTINE FILTER_YY(t,u,r,fval,fva,fvb,fvc,fvd,fve,&
                          s,vf,vs,vw,nx,ny,nz,npaire)
       use decomp_2d, only : mytype
       integer :: nx,ny,nz,npaire
       real(mytype), dimension(nx,ny,nz) :: t,u,r
       real(mytype), dimension(nx,nz):: s
       real(mytype), dimension(ny):: vf,vs,vw
       real(mytype)               :: fval,fva,fvb,fvc,fvd,fve
     END SUBROUTINE FILTER_YY
     SUBROUTINE FILTER_ZZ(t,u,r,fval,fva,fvb,fvc,fvd,fve,&
                          s,vf,vs,vw,nx,ny,nz,npaire)
       use decomp_2d, only : mytype
       integer :: nx,ny,nz,npaire
       real(mytype), dimension(nx,ny,nz) :: t,u,r
       real(mytype), dimension(nx,ny):: s
       real(mytype), dimension(nz):: vf,vs,vw
       real(mytype)               :: fval,fva,fvb,fvc,fvd,fve
     END SUBROUTINE FILTER_ZZ
  END INTERFACE
  ...
  !viscous filter
  PROCEDURE (FILTER_XX) filxx_00
  PROCEDURE (FILTER_XX), POINTER :: filxx,filxxS
  PROCEDURE (FILTER_YY) filyy_00
  PROCEDURE (FILTER_YY), POINTER :: filyy,filyyS
  PROCEDURE (FILTER_ZZ) filzz_00
  PROCEDURE (FILTER_ZZ), POINTER :: filzz,filzzS
  ...
  real(mytype),allocatable,dimension(:) :: xp,zp
...
module param
  ...
  !====DEBUG
  integer :: new_rec !New reconstruction (through periodicity, for pipe flow)
...
module complex_geometry
  ...
  real(mytype)                              :: tol
  integer :: nxraf,nyraf,nzraf,nraf,nobjmax,iepm
...
! Viscous filtering coefficients 
module viscfiX
  use decomp_2d, only : mytype
  !velocity
  real(mytype) :: fvaix, fvbix, fvcix, fvdix, fveix, fvalx
  !scalar
  real(mytype),allocatable,dimension(:) :: fscaix, fscbix, fsccix, fscdix, fsceix, fscalx
  !scalar CHT (solid coefficients)
  real(mytype),allocatable,dimension(:) :: fscaix_s, fscbix_s, fsccix_s, fscdix_s, fsceix_s, fscalx_s
end module viscfiX
!
module viscfiY
  use decomp_2d, only : mytype
  !velocity
  real(mytype) :: fvajy, fvbjy, fvcjy, fvdjy, fvejy, fvaly
  !scalar
  real(mytype),allocatable,dimension(:) :: fscajy, fscbjy, fsccjy, fscdjy, fscejy, fscaly
  !scalar CHT (solid coefficients)
  real(mytype),allocatable,dimension(:) :: fscajy_s, fscbjy_s, fsccjy_s, fscdjy_s, fscejy_s, fscaly_s
end module viscfiY
!
module viscfiZ
  use decomp_2d, only : mytype
  !velocity
  real(mytype) :: fvakz, fvbkz, fvckz, fvdkz, fvekz, fvalz
  !scalar
  real(mytype),allocatable,dimension(:) :: fscakz, fscbkz, fscckz, fscdkz, fscekz, fscalz
  !scalar CHT (solid coefficients)
  real(mytype),allocatable,dimension(:) :: fscakz_s, fscbkz_s, fscckz_s, fscdkz_s, fscekz_s, fscalz_s
end module viscfiZ
