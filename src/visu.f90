...
contains
  ...
  subroutine write_snapshot(rho1, ux1, uy1, uz1, pp3, phi1, ep1, itime)
    ...
    use param, only : ivisu, ioutput, nrhotime, ilmn, iscalar, iibm, sync_vel_needed
	...
	character(len=5), parameter :: foldname_p='data/'
...
990    format(a,'ux',I4.4)
       write(filename, 990) trim(foldname_p), itime/ioutput
       ...
       !call fine_to_coarseV(1,ta1,uvisu)
991    format(a,'uy',I4.4)
       write(filename, 991) trim(foldname_p), itime/ioutput
       call decomp_2d_write_one(1,ta1,filename,2)
       !===DEBUG
       !call decomp_2d_write_one(1,uvisu,filename,2)
       ...
       !call fine_to_coarseV(1,ta1,uvisu)
992    format(a,'uz',I4.4)
       write(filename, 992) trim(foldname_p), itime/ioutput
       call decomp_2d_write_one(1,ta1,filename,2)
       !===DEBUG
       !call decomp_2d_write_one(1,uvisu,filename,2)
       ...
       !call fine_to_coarseV(1,ta1,uvisu)
993    format(a,'pp',I4.4)
       write(filename, 993) trim(foldname_p), itime/ioutput
       call decomp_2d_write_one(1,ta1,filename,2)
       !===DEBUG
       !call decomp_2d_write_one(1,uvisu,filename,2)
...
996       format('phi',i2.2,i3.3,i4.4)
596       format('phis',i2.2,i3.3,i4.4)
          icht=0
          do is = 1, numscalar
             if (iibm==2) then
                ta1(:,:,:) = (one - ep1(:,:,:)) * phi1(:,:,:,is)
