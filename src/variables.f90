...
module var
  ...
  real(mytype), save, allocatable, dimension(:,:,:,:) :: phis1, phis2, phis3
  real(mytype), save, allocatable, dimension(:,:,:,:,:) :: dphis1
  real(mytype), save, allocatable, dimension(:,:,:) :: px1, py1, pz1
  real(mytype), save, allocatable, dimension(:,:,:) :: ep1, epm, diss1, pre1, depo, depof, kine
  ...
  !arrays for budget statistic collection
  real(mytype), save, allocatable, dimension(:,:,:) :: dudx_mean,dvdx_mean,dwdx_mean,dpdx_mean
  real(mytype), save, allocatable, dimension(:,:,:) :: dudy_mean,dvdy_mean,dwdy_mean,dpdy_mean
  real(mytype), save, allocatable, dimension(:,:,:) :: dudz_mean,dvdz_mean,dwdz_mean,dpdz_mean

  real(mytype), save, allocatable, dimension(:,:,:,:) :: dphidx_mean,dphidy_mean,dphidz_mean
  real(mytype), save, allocatable, dimension(:,:,:,:) :: dphidx_dphidx_mean,dphidy_dphidy_mean,dphidz_dphidz_mean
  real(mytype), save, allocatable, dimension(:,:,:,:) :: dphidxx_mean,dphidyy_mean,dphidzz_mean
  real(mytype), save, allocatable, dimension(:,:,:,:) :: phi_dphidxx_mean,phi_dphidyy_mean,phi_dphidzz_mean

  real(mytype), save, allocatable, dimension(:,:,:,:) :: dphisdx_mean,dphisdy_mean,dphisdz_mean
  real(mytype), save, allocatable, dimension(:,:,:,:) :: dphisdx_dphisdx_mean,dphisdy_dphisdy_mean,dphisdz_dphisdz_mean
  ...
  real(mytype), save, allocatable, dimension(:,:,:) :: phismean,phisphismean
  !====DEBUG
  !real(mytype), save, allocatable, dimension(:,:,:) :: umean,vmean,wmean,pmean,uumean,vvmean,wwmean,uvmean,uwmean,vwmean,tmean
  !real(mytype), save, allocatable, dimension(:,:,:,:) :: phimean,phiphimean,uphimean,vphimean,wphimean
  ...
  real(mytype), save, allocatable, dimension(:,:,:) :: dudxsum,utmapsum		! Tolstoy validation
  real(mytype), save, allocatable, dimension(:,:) :: uttmapsum		! Tolstoy validation
...
contains
  ...
  subroutine init_variables
    ...
    !Conjugate Heat Transfer (solid temp field)
    if (iscalar.ne.0) then
        ...
        enddo
        if (numcht.ne.0) then
            !allocate(phis1(xstart(1):xend(1),xstart(2):xend(2),xstart(3):xend(3),1:numcht)) !global indices
            allocate(phis1(xsize(1),xsize(2),xsize(3),1:numcht))
            allocate(phis2(ysize(1),ysize(2),ysize(3),1:numcht))
            allocate(phis3(zsize(1),zsize(2),zsize(3),1:numcht))
        endif
    endif
    ...
    if (iepm.eq.1) call alloc_x(epm) !One-mesh retracted epsilon matrix
    if (ilmn) then
      call alloc_x(mu1)
      mu1(:,:,:) = one
    endif
    ...
    if (numcht.ne.0) then
        allocate (phismean(xstS(2):xenS(2),xstS(3):xenS(3),numcht))
        allocate (phisphismean(xstS(2):xenS(2),xstS(3):xenS(3),numcht))
    endif
    !====DEBUG
    ...
    if (itype.eq.itype_pipe) then
        ...
        !!====DEBUG
        !allocate (uphimean(xstS(1):xenS(1),xstS(2):xenS(2),xstS(3):xenS(3),numscalar))
		
		!Tolstoy-allocate statistics for validation
		allocate (dudxsum(xstS(1):xenS(1),xstS(2):xenS(2),xstS(3):xenS(3)))
		allocate (utmapsum(xstS(1):xenS(1),xstS(2):xenS(2),xstS(3):xenS(3)))
		! Tolstoy-allocate statistics for validation ends
    endif
    ...
    !module viscous filter
    !velocity
    allocate(vfx(nx),vcx(nx),vbx(nx),vsx(nx),vwx(nx),vfxp(nx),vsxp(nx),vwxp(nx))
    allocate(vfy(ny),vcy(ny),vby(ny),vsy(ny),vwy(ny),vfyp(ny),vsyp(ny),vwyp(ny))
    allocate(vfz(nz),vcz(nz),vbz(nz),vsz(nz),vwz(nz),vfzp(nz),vszp(nz),vwzp(nz))
    !scalar
    allocate(fscaix(numscalar), fscbix(numscalar), fsccix(numscalar),&
             fscdix(numscalar), fsceix(numscalar), fscalx(numscalar))
    allocate(fscajy(numscalar), fscbjy(numscalar), fsccjy(numscalar),&
             fscdjy(numscalar), fscejy(numscalar), fscaly(numscalar))
    allocate(fscakz(numscalar), fscbkz(numscalar), fscckz(numscalar),&
             fscdkz(numscalar), fscekz(numscalar), fscalz(numscalar))
    allocate(vscfx(nx,numscalar),vscsx(nx,numscalar),vscwx(nx,numscalar))
    allocate(vsccx(nx),vscbx(nx),vscfxp(nx),vscsxp(nx),vscwxp(nx))
    allocate(vscfy(ny,numscalar),vscsy(ny,numscalar),vscwy(ny,numscalar))
    allocate(vsccy(ny),vscby(ny),vscfyp(ny),vscsyp(ny),vscwyp(ny))
    allocate(vscfz(nz,numscalar),vscsz(nz,numscalar),vscwz(nz,numscalar))
    allocate(vsccz(nz),vscbz(nz),vscfzp(nz),vscszp(nz),vscwzp(nz))
    !Solid coefficients
    if (numcht.ne.0) then !CHT
        allocate(fscaix_s(numscalar), fscbix_s(numscalar), fsccix_s(numscalar),&
                 fscdix_s(numscalar), fsceix_s(numscalar), fscalx_s(numscalar))
        allocate(fscajy_s(numscalar), fscbjy_s(numscalar), fsccjy_s(numscalar),&
                 fscdjy_s(numscalar), fscejy_s(numscalar), fscaly_s(numscalar))
        allocate(fscakz_s(numscalar), fscbkz_s(numscalar), fscckz_s(numscalar),&
                 fscdkz_s(numscalar), fscekz_s(numscalar), fscalz_s(numscalar))
        allocate(vscfx_s(nx,numscalar),vscsx_s(nx,numscalar),vscwx_s(nx,numscalar))
        allocate(vscfy_s(ny,numscalar),vscsy_s(ny,numscalar),vscwy_s(ny,numscalar))
        allocate(vscfz_s(nz,numscalar),vscsz_s(nz,numscalar),vscwz_s(nz,numscalar))
    endif
    ...
    if (numcht.ne.0) then
        allocate(dphis1(xstart(1):xend(1),xstart(2):xend(2),xstart(3):xend(3),ntime,1:numcht)) !global indices
    endif
