...
subroutine parameter(input_i3d)
  ...
  if (numscalar.ne.0) then
     ...
     allocate(itbc(numscalar))
     allocate(g1(numscalar)) !if CHT, fluid-to-solid ratio of thermal diffusivities
     allocate(g2(numscalar)) !if CHT, solid-to-fluid ratio of thermal conductivities
     itbc(:) = 1
     g1(:) = 1
     g2(:) = 1
     ri(:) = zero
  ...
  if(ilesmod.ne.0) read(10, nml=LESModel); rewind(10)
  if (itype.eq.itype_tbl) read(10, nml=Tripping); rewind(10)
  ...
  if (nrank==0) then
     ...
     if (ilmn) then
        ...
        write(*,"(' Prandtl number Pr  : ',F15.8)") prandtl
     endif
     ...
     if (iscalar.eq.0) then
        ...
        do is=1, numscalar
           write (*,"(' Scalar             : #',I2)") is
           if (itbc(is).eq.1) then
             write (*,"(' Thermal BC         : MBC')")
           elseif (itbc(is).eq.2) then
             write (*,"(' Thermal BC         : IF ')")
           elseif (itbc(is).eq.3) then
             write (*,"(' Thermal BC         : CHT')")
             write (*,"(' Diff. ratio g1     : ',F7.4)") g1(is)
             write (*,"(' Cond. ratio g2     : ',F7.4)") g2(is)
             write (*,"(' Act. ratio K       : ',F7.4)") 1./(g2(is)*sqrt(g1(is)))
           endif
