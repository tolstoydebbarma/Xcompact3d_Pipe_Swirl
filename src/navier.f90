...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  SUBROUTINE solve_poisson(pp3, px1, py1, pz1, rho1, ux1, uy1, uz1, ep1, drho1, divu3)
    ...
    IF (iepm.eq.0) then !If one-mesh retracted epsilon (epm) is used
        CALL divergence(pp3(:,:,:,1),rho1,ux1,uy1,uz1,ep1,drho1,divu3,nlock)
    ELSE
        CALL divergence(pp3(:,:,:,1),rho1,ux1,uy1,uz1,epm,drho1,divu3,nlock)
    ENDIF
    IF (ilmn.AND.ivarcoeff) THEN
       dv3(:,:,:) = pp3(:,:,:,1)
    ENDIF
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine divergence (pp3,rho1,ux1,uy1,uz1,ep1,drho1,divu3,nlock)
    ...
    if ((nrank==0).and.(nlock.gt.0).and.(mod(itime, ilist) == 0 .or. itime == ifirst .or. itime==ilast)) then
       if (nlock==2) then
  ...
  subroutine pre_correc(ux,uy,uz,ep)
    ...
    USE var
    USE MPI
    USE pipe, ONLY: pipe_flrt, pipe_blkt, phiw_cht, phis_condeq
    USE pipe, ONLY: chtlagpoly_s, chtlagpolz_s
    USE pipe, ONLY: phiws2, phiws3

    implicit none

    real(mytype),dimension(xsize(1),xsize(2),xsize(3)) :: ux,uy,uz,ep
    integer :: i,j,k,is,icht
    ...
    if (itype.eq.itype_pipe) then
        ...
        icht=0
        do is=1,numscalar
            call pipe_blkt(ux1,phi1(:,:,:,is),ep1,is)
            if (itbc(is).eq.3) then !CHT
                icht=icht+1
                call phiw_cht(phi1(:,:,:,is),phis1(:,:,:,icht),ep1,is,icht,2,1)
